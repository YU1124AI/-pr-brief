      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
          <div className="space-y-8 min-w-0">
            ...
            <FeedSection
              title="MarkeZine — マーケティング戦略"
              icon={iconAd()}
              items={data?.markezine || []}
              loading={loading}
              cols={3}
            />

            <FeedSection
              title="AdverTimes — 広告・クリエイティブ"
              icon={iconAd()}
              items={data?.adverTimes || []}
              loading={loading}
              cols={3}
            />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {loading
                  ...
                  : (data?.dailyIdeas || []).map((idea, i) => (
                      <div
                        key={i}
                        className="glass glass-hover rounded-2xl p-4 flex flex-col gap-2.5"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded-full ${
                              TAG_COLORS[idea.tag] || 'tag-pr'
                            }`}
                          >
                            {idea.tag}
                          </span>
        ...
        <footer className="mt-12 pt-6 border-t border-white/5 flex items-center justify-between text-xs text-gray-700 flex-wrap gap-2">
          ...
          <span>
            PR TIMES / PR EDGE / Web担当者Forum /
            MarkeZine / AdverTimes / Yahoo!ニュース /
            朝日新聞 / X Trends
          </span>
        </footer>
      </div>
